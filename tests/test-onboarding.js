...
(async () => {
  ...
  const cas = await page.evaluate(() => {
    ...
    return {
      ...
      batimentsAvant:     test('batiments',  { verdure: 14999 }),
      batimentsApres:     test('batiments',  { verdure: 15000 }),
      batimentsCumul:     test('batiments',  { verdure: 0, totalEarned: 1e9 }),
      specialAvant:       test('special',    { verdure: 2.4e6 }),
      specialApres:       test('special',    { verdure: 2.5e6 }),
      rechercheSansBat:   test('recherche',  { totalPlayTimeSec: 3600 }),
      rechercheSansTemps: test('recherche',  { tabsSeen: { batiments: true }, totalPlayTimeSec: 1799 }),
      rechercheOk:        test('recherche',  { tabsSeen: { batiments: true }, totalPlayTimeSec: 1800 }),
  ...
  const attendu = {
    ...
    rechercheSansBat: false, rechercheSansTemps: false, rechercheOk: true,
